...
//
// errors.ts is parsed as text (comments stripped first) rather than imported:
// importing it would couple this stage-3 check to the build state of
// @mobile-surfaces/traps. Identifier-presence parsing is sufficient because
// the failure consequence is low and the file's structure is regular.
...
// Strip block and line comments so a `case` or `class` inside a comment
// cannot satisfy (or, for the orphan check, falsely trip) the gate. String
// literals in the scanned regions of errors.ts never contain `//` or `/*`,
// so a full lexer is unnecessary here.
...
const src = stripComments(fs.readFileSync(ERRORS_TS, "utf8"));
