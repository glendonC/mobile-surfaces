...
public enum MSTraps {
  public static let all: [String: MSTrapBinding] = [
    ...
    "MS041": MSTrapBinding(
      ...
      fix: "Add `schemaVersion: z.literal(\"<current>\")` as the first property of every projection-output schema in packages/surface-contracts/src/schema.ts. The literal value must match the snapshot schema's `schemaVersion` literal (the canonical wire-format generation). The matching Swift Codable structs in apps/mobile/targets/_shared/MobileSurfacesSurfaceSnapshots.swift and apps/mobile/targets/notification-content/MobileSurfacesNotificationContentEntry.swift are generated from those schemas by scripts/generate-surface-swift.mjs; run pnpm surface:codegen after the schema edit. The widget's `readSnapshot` helper then attempts the `{ schemaVersion: String }` probe first, and a mismatch renders the version-mismatch placeholder view instead of failing silently.",
