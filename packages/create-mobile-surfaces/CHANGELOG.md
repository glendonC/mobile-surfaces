...
- Updated dependencies [c347e54]
  - @mobile-surfaces/validators@8.0.0
...
- Updated dependencies [b6f4eb5]
  - @mobile-surfaces/validators@3.2.0
...
- Updated dependencies [e582058]
  - @mobile-surfaces/validators@3.1.1

## 3.1.0

### Minor Changes

- Bundled template picks up the 3.1.0 SDK ergonomics additions. Scaffolds generated by `npm create mobile-surfaces@latest` now ship:

  - `scripts/send-apns.mjs` with the new `--print`, `--describe`, and `--json` flags. `--print` emits payload bytes without minting a JWT or opening a socket. `--describe` emits a full request preview (path, headers, payload size, MS011 ceiling status). `--json` emits the response (or dry-run preview) as a single structured JSON record. Dry-run flags skip the APNs env-var check so CI pipelines can preflight payloads without auth keys configured.

  - Harness refresh buttons for the lockAccessory and StandBy surfaces, gated by their respective surface markers. Users who opt the surfaces out at scaffold time get the corresponding code stripped from the harness, fixture exports, and surfaceStorage imports — no dangling references.

  - The `lockAccessoryWidget` and `standbyWidget` surface ids participate in the strip pipeline alongside `homeWidget` and `controlWidget`; multi-id markers around shared scaffolding (the `surfaceStorage` import block, the `setSurfaceStatus` state hook, the surface-status section) now require _all_ four surfaces to be off before the shared code is removed.

### Patch Changes

- Updated dependencies
  - @mobile-surfaces/validators@3.1.0
...
- Updated dependencies [870f437]
  - @mobile-surfaces/validators@2.1.1
