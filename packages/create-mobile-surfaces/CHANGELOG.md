...
- Bundled template picks up the 3.1.0 SDK ergonomics additions. Scaffolds generated by `npm create mobile-surfaces@latest` now ship:
  ...
  - The `lockAccessoryWidget` and `standbyWidget` surface ids participate in the strip pipeline alongside `homeWidget` and `controlWidget`; multi-id markers around shared scaffolding (the `surfaceStorage` import block, the `setSurfaceStatus` state hook, the surface-status section) now require *all* four surfaces to be off before the shared code is removed.
