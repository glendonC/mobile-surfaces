...
final class LiveActivityCodableBridgeTests: XCTestCase {
    ...
    func testLiveActivityErrorDescriptionMatchesContractedCodes() {
        XCTAssertEqual(LiveActivityError.unsupportedOS.description, "ACTIVITY_UNSUPPORTED_OS")
        XCTAssertEqual(LiveActivityError.notFound.description, "ACTIVITY_NOT_FOUND")
        XCTAssertEqual(LiveActivityError.unsupportedFeature.description, "ACTIVITY_UNSUPPORTED_FEATURE")
        XCTAssertEqual(
            LiveActivityError.decodingFailed(reason: "field x missing").description,
            "ACTIVITY_DECODE_FAILED: field x missing"
        )
        XCTAssertEqual(
