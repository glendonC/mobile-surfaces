...
- a05688f: README accuracy fix. The "What this package does not do" section claimed three things the package actually does:
...
- `@mobile-surfaces/live-activity` moves to independent versioning. It is no longer co-versioned with the linked release group (`surface-contracts` + `validators` + `traps`); its dependency on `@mobile-surfaces/surface-contracts` and `@mobile-surfaces/traps` is now declared as `workspace:^`. See `apps/site/src/content/docs/stability.md` for the versioning charter.

- The Swift `LiveActivityError` enum conforms to `MSTrapBound` from `MobileSurfacesTraps.swift`. Native rejections carry the matching trap id in a stable `[trap=MSXXX url=...]` suffix on the message; the JS-side adapter parses the suffix into a `LiveActivityNativeError extends MobileSurfacesError` whose `trapId` and `docsUrl` are populated. `decodingFailed` binds to MS003 (Lock Screen decode mismatch); `unsupportedOS` / `unsupportedFeature` bind to MS012 (iOS deployment target).

- The native module ships `MobileSurfacesTraps.swift` byte-identically with `packages/traps/swift/` and `apps/mobile/targets/_shared/`. The byte-identity gate (MS040, `scripts/check-traps-swift-byte-identity.mjs`) catches drift; the canonical copy is generated by `scripts/generate-traps-package.mjs` from `data/traps.json`.

### Minor Changes

- The `apps/mobile/targets/notification-content/generated.entitlements` file is now codegened by `scripts/generate-app-group-constants.mjs` and participates in the MS013 App Group identity gate. Hand edits revert on the next `pnpm surface:codegen`.

- `StagePill.stage` no longer flags an unused-parameter warning in `MobileSurfacesLiveActivity.swift`; the parameter is consumed in the rendered SwiftUI text.

- `getPushToStartToken()` returns `nil` before the first AsyncSequence emission (regression test added in `ObserverRegistryTests.swift`).

- The `LiveActivityModule.swift` `OnStopObserving` aggressive-cancel block carries an inline comment so future contributors don't re-regress; the cancel-on-detach semantics are load-bearing for token-store discipline (MS039).
...
- Linked-group bump for the v4 schema release in `@mobile-surfaces/surface-contracts`. The JS-facing adapter API is unchanged; the native bridge picks up several reliability fixes that change observable behavior on the rejection path.

- `LiveActivityError` widens from a flat raw-value enum to associated-value cases (`decodingFailed(reason:)`, new `encodingFailed(reason:)`) and conforms to `CustomStringConvertible` + `LocalizedError`. The JS-side `error.code` string ("ACTIVITY_DECODE_FAILED", etc.) is preserved as the `description` prefix, but the rejection message now carries the underlying Codable failure reason instead of a bare code. Bridge consumers reading `error.message` see strictly more detail; consumers matching on the `code` prefix keep working.

### Minor Changes

- The native observer registry caches the latest push-to-start token Apple emitted through the AsyncSequence and exposes it through `getPushToStartToken()`. JS callers polling for the value after the event stream has already delivered it no longer receive a misleading `nil`. Token rotations (MS020 — treat-latest-as-authoritative) overwrite the cache; clearing happens only on bridge teardown.

- All registered AsyncSequence tasks now cancel on `OnStopObserving` (every JS listener detached) in addition to module `deinit`. Previously the registry leaked active sequence tasks across fast-refresh reloads.
