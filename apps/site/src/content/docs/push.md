...
The option is named `_unsafeRetryOverride` because changing it usually goes wrong: the defaults are tuned against MS015's iOS budget rules and the priority-aware stretch. The legacy name `retryPolicy` still works but logs a one-time deprecation warning per process; it will be removed in @mobile-surfaces/push@9.0.
