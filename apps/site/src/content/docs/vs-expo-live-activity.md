...
| Catalog of silent-failure modes (<!-- catalog-stats:userFacing -->34<!-- /catalog-stats:userFacing --> documented iOS + wire traps, plus <!-- catalog-stats:maintenance -->7<!-- /catalog-stats:maintenance --> catalog-maintenance gates; <!-- catalog-stats:prGated -->24<!-- /catalog-stats:prGated --> PR-gated in `surface:check`) | no | no | no | yes |
...
**3. The catalog catches what the bridge can't see.** <!-- catalog-stats:userFacing -->34<!-- /catalog-stats:userFacing --> invariants in `data/traps.json` enumerate the silent-failure modes a downstream consumer can actually trip: App Group identity mismatches that render placeholder forever, push tokens from the wrong environment that fail with 400 BadDeviceToken, the `.push-type.liveactivity` topic suffix that has to be appended exactly once, the iOS 18 broadcast capability that's invisible until the first send fails. <!-- catalog-stats:prGated -->24<!-- /catalog-stats:prGated --> of those rules are PR-gated by scripts in `surface:check`; another <!-- catalog-stats:runtime -->9<!-- /catalog-stats:runtime --> surface as typed errors from the push SDK at call time (payload size, missing env vars, expired provider tokens, channel management); the rest are advisory or toolchain-preflight notes. A further <!-- catalog-stats:maintenance -->7<!-- /catalog-stats:maintenance --> catalog-maintenance gates protect the registry itself (deprecation prose, CHANGELOG-on-major, catalog-stats sync) and are not counted in the user-facing total. To run the catalog against a project that is not this repo, clone Mobile Surfaces and use `pnpm surface:audit --root <path>`; the static and config gates that apply to a foreign project run against it directly.
