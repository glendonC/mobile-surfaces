...
6. **Duplicated ActivityKit attribute files** are generated from Zod, enforced by two gates:
   ...
   Both files are emitted by `scripts/generate-surface-swift.mjs` from `liveSurfaceActivityContentState` and `liveSurfaceStage` in `packages/surface-contracts/src/schema.ts`. `surface:check` gates the codegen with `--check` at stage 2 (drift fails CI), and `scripts/check-activity-attributes.mjs` at stage 3 redoes the byte-identity + Zod parity check as defense in depth.
...
`LiveSurfaceSnapshot` carries a `schemaVersion: "5"` literal and a top-level `kind` discriminator. The union is strict. The package validates strictly against v5; there is no multi-version codec. The deprecation history and the policy for migrating stored payloads live in [`schema.md`](/docs/schema).
