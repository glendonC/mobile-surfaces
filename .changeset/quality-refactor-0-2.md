...
Make the contract package consumable from plain Node backends and tighten correctness across the board.
...
- Replace the anemic `schema.json` with Zod v4 as the single source of truth: ships `assertSnapshot` / `safeParseSnapshot` / `liveSurfaceSnapshot`, generates a real JSON Schema (`unpkg.com/@mobile-surfaces/surface-contracts@0/schema.json`), and adds a `schemaVersion: "0"` discriminator with a documented evolution policy.
